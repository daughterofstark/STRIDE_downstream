...
> **Status: Stages S0 and S1A implemented.**
...
> data layer* built on top of S0: canonical residue objects, domain summaries,
> replicate availability, and cross-serotype conservation mapping. Both stages
> contain **no statistics, no ranking, no mechanism inference, and no figures** —
> those belong to later stages (S1B+), which are intentionally not implemented
> here. See [`docs/s1a.md`](docs/s1a.md) for the S1A tables and their consumers.
...
├── docs/                    # architecture, data model, usage, s1a
