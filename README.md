...
> **Status: Stage S0 only.**
> S0 = ingestion, validation, and canonical data-layer construction. It builds
> the trustworthy tables that every later stage will consume. It contains **no
> biological interpretation, no statistics, and no figures** — those belong to
> later stages (S1+), which are intentionally not implemented here.
