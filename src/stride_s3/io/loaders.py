"""Loader for the S3 input.

S3 consumes the S0 STRIDE table (``stride_table.parquet``) — the profile, with
every locus at every scale. It never re-reads the raw STRIDE CSV/JSON files or MD
trajectories. This loader reads the parquet, asserts the columns S3 depends on
...
def load_stride_table(path: str | Path) -> pd.DataFrame:
    """Load the S0 STRIDE table, asserting the columns S3 depends on."""
