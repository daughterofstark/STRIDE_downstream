...
> **Status: Stages S0, S1A and S1B implemented.**
...
> per-hierarchy, and per-serotype structural annotations. All three stages
> contain **no statistics, no ranking, no mechanism inference, and no figures** —
> those belong to later stages (S2+), which are intentionally not implemented
> here. See [`docs/s1a.md`](docs/s1a.md) and [`docs/s1b.md`](docs/s1b.md) for the
...
├── docs/                    # architecture, data model, usage, s1a, s1b
