...
> **Status: Stages S0, S1A, S1B, S2, S3, S4, S5, S6 and S7 implemented.**
...
> approximated.
> **S7** = the *reporting layer* — the terminal stage. It consumes the reduced
> outputs of S2–S6 and assembles the design's publication figures (**F1–F8**) and
> manuscript tables (**T1–T5**), deterministically and provenance-stamped. It
> performs **no new statistics and no inference**: every value is read from a prior
> stage's table and merely selected, joined, ordered, and formatted, and it never
> reads the raw STRIDE files. All eight figures and five tables are fully supported
> by the existing outputs; the replicate layer (S6) feeds no design figure/table,
> so its blocked-analysis ledger is surfaced under the report's limitations rather
> than fabricated into a figure. Figures are emitted as deterministic,
> dependency-free **SVG** (plus prepared-data CSV/Parquet); tables as
> CSV/Parquet/Markdown; with a summary JSON carrying the input SHA-256 digests,
> the provisional ρ\\*, `calibrated = false`, and the validation results.
> None of these stages contain **statistics, ranking, mechanism inference, or
> analysis** beyond S2/S3/S4's structural reduction, S5's descriptive
> cross-serotype tallies, S6's replicate-axis diagnostics, and S7's deterministic
> reporting — calibrated inference belongs to later stages (S8+), which are
...
> [`docs/s4.md`](docs/s4.md), [`docs/s5.md`](docs/s5.md),
> [`docs/s6.md`](docs/s6.md), and [`docs/s7.md`](docs/s7.md) for the tables and
> their consumers.
...
# finally assemble the S7 publication figures (F1-F8) and manuscript tables (T1-T5)
...
│   └── stride_s7/           # S7: the reporting layer (figures & tables)
...
├── docs/                    # architecture, data model, usage, s1a, s1b, s2, s3, s4, s5, s6, s7
...
- [`docs/s7.md`](docs/s7.md) — the S7 reporting layer (figures F1-F8, tables T1-T5)
